    ...
    if ($conn->query($sql1) === TRUE && $conn->query($sql2) === TRUE) {
        ...
        echo "<script>
                document.addEventListener('DOMContentLoaded', function () {
                    ...
                    }, 3000); // Set the timeout duration in milliseconds (5 seconds in this case)
                });
              </script>";
    } else {
...
<header>
    ...
    <main class="container mt-4">
        ...
        <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" class="mt-4">
            ...
            <div class="mb-3">
                ...
                <input type="number" name="quantity" id="quantity" class="form-control" min="0.1" step="0.01">
            </div>
        ...
        <!-- <div class="mt-4">
          //  <?php 
           // $sql = "SELECT * FROM raw_materialadded";
          //  $result = $conn->query($sql);

            // Check if there are rows returned
          //  if ($result->num_rows > 0) {
                // Output data of each row
          //      echo "<table class='table table-bordered table-striped'><thead><tr><th>S.no.</th><th>Material Name</th><th>Material Type</th><th>Quantity</th><th>Date&Time</th></tr></thead><tbody>";
           //     while($row = $result->fetch_assoc()) {
           //         echo "<tr><td>".$row["sno"]."</td><td>".$row["materialNamea"]."</td><td>".$row["materialTypea"]."</td><td>".$row["quantity"]."</td><td>".$row["timestamp"]."</td></tr>";
           //     }
          //      echo "</tbody></table>";
         //   } else {
          //      echo "0 results";
    ...
    if ($result->num_rows > 0) {
        // Output data of each row
        echo "<table class='table table-bordered table-striped'><thead><tr><th>ID.</th><th>Material Name</th><th>Material Type</th><th>Quantity</th><th>Date&Time</th></tr></thead><tbody>";
        while($row = $result->fetch_assoc()) {
            echo "<tr><td>".$row["sno"]."</td><td>".$row["materialNamea"]."</td><td>".$row["materialTypea"]."</td><td>".$row["quantity"]."</td><td>".$row["timestamp"]."</td></tr>";
        }
        echo "</tbody></table>";
